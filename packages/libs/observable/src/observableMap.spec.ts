...
describe('observableMap', () => {
  ...
  it('should emit a new value to subscribers when a value is set', () => {
    const map = observableMap(new Map([['a', 1]]));
    const subscriber = jest.fn();

    map.subscribe({ next: subscriber });
    expect(subscriber).toHaveBeenCalledTimes(1);
    expect(subscriber).toHaveBeenCalledWith(new Map([['a', 1]]));
    map.set('b', 2);
    expect(subscriber).toHaveBeenCalledWith(new Map([['a', 1], ['b', 2]]));
  });

  it('should emit a new value to subscribers when a value is deleted', () => {
    const map = observableMap(new Map([['a', 1], ['b', 2]]));
    const subscriber = jest.fn();
