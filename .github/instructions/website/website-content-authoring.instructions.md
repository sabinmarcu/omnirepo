...
- Built-in code annotations include `!mark`, `!diff +`, `!diff -`, `!word-wrap`, `!line-numbers`, `!no-line-numbers`, `!shell-prompt`, `!no-shell-prompt`, `!copy`, `!no-copy`, `!language`, `!no-language`, and inline `!fold[/pattern/]`. Prefix an annotation comment with `!noop` to render the following marker literally without the `noop` prefix, for example `// !noop diff -` renders as `// !diff -`. Shell blocks show `$` prompts and copy controls by default; use `!no-shell-prompt` or `!no-copy` to hide them, or use `!shell-prompt` and `!copy` to enable them on another language. Non-shell blocks show line numbers by default; use `!no-line-numbers` to hide them, or `!line-numbers` to enable them explicitly. Code blocks show their current-language control by default; use `!no-language` to hide it, or `!language` to show it explicitly. When both forms are present, the positive annotation takes precedence. Plain fence metadata such as `` ```ts example.ts `` renders as a file name.
