...
```mermaid
graph LR
    Source["Source Video"] --> Library["Cutrail Library\n(Fingerprint & Metadata)"]
    Library --> Editor["Independent Editor Window"]
    Editor --> Tracks["Audio Track Selection\n(Mix or Mute Streams)"]
    Editor --> Trimming{"Trim Accuracy"}
    Trimming -->|Quick| Fast["Fast Trimming\n(Keyframe Boundary)"]
    Trimming -->|Accurate| Precise["Accurate Trimming\n(Exact Boundary Frame)"]
    Fast & Precise & Tracks --> FFmpeg["FFmpeg External Process"]
    FFmpeg --> Output["Output Clip + Linked Metadata"]
```
