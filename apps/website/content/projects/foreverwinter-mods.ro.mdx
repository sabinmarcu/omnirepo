...
```mermaid
stateDiagram-v2
    [*] --> SteamDiscovery: Auto-detectare la pornire / --path
    SteamDiscovery --> ArchiveInspect: fw-mods install archive
    
    state ArchiveInspect {
        [*] --> New: Triplet nou
        [*] --> Update: Marcaj mai recent
        [*] --> Older: Marcaj mai vechi
        [*] --> Installed: Triplet identic
    }
    
    ArchiveInspect --> Enabled: Instalare triplet (.pak, .ucas, .utoc)
    Enabled --> Disabled: Comutare în CLI (adaugă .disabled)
    Disabled --> Enabled: Comutare în CLI (restabilește extensiile)
```
