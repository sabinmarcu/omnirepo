...
async function ensureWorkspaceProtocol({ Yarn }) {
  for (const dependency of Yarn.dependencies()) {
    if (Yarn.workspace({ ident: dependency.ident }) && dependency.range !== WORKSPACE_PROTOCOL_RANGE) {
...
async function ensureFieldConsistency({ Yarn }) {
  for (const workspace of Yarn.workspaces()) {
    if (FIELD_IGNORE_LIST.includes(`${workspace.ident}`)) {
      continue;
    }
